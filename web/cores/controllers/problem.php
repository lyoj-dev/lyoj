...
class Problem_Controller {
    ...
    static function ListProblemByNumber(float $l=1,float $r=1e18,string $key="",array|null $tag,array|null $diff,float& $num):array|null {
        ...
        $array=self::$db->Query("SELECT * FROM problem WHERE ".($key!=""?"name LIKE '%$key%' AND ":"")."id>=0".($permission<=1?" AND banned=0 AND hidden=0":"").(count($diff)?$diffs:"")." ORDER BY id");
     ...
     * @param int $contest=0 题目所属比赛
     * @return int
     */
    static function CreateProblem(
        ...
        array|null $tags=null,int $contest=0):int {
        ...
        $sql="INSERT INTO problem (id,name,bg,descrip,input,output,cases,hint,contest,difficult,hidden,banned) VALUES ($id,'$title','$background'".
        ",'$description','$input','$output','$sample','$hint',$contest,$difficult,0,0)";
        self::$db->Execute($sql);
        return $id;
    }
     ...
     * @return void
     */
    static function UpdateProblem(
        int $pid,string $background,string $description,string $input,string $output,
        string $input_file,string $output_file,string $sample,string $hint,
        ...
        string $spj_param="",array|null $tags=null,int $contest=0):void {
        Problem_Controller::JudgeProblemExist($pid);
        $array=self::$db->Query("SELECT * FROM problem WHERE id=$pid");
        ...
        if ($contest!="") self::$db->Execute("UPDATE problem SET contest='$contest' WHERE id=$pid");
