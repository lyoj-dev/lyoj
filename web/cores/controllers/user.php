...
class Login_Controller {
     ...
     * @return string
     */
    static function CreateToken(string $email):string {
        $uid=self::$user_controller->GetEmailId($email);
        if ($uid==0) API_Controller::error_email_not_exist($email);
        $token=bin2hex(openssl_random_pseudo_bytes(50));
        self::$db->Execute("UPDATE user SET token='$token' WHERE id=$uid");
        return $token;
    }
     ...
     * @return bool
     */
    function CheckToken(string $email,string $token):bool {
        $arr=self::$db->Query("SELECT * FROM user WHERE email='$email' AND token='$token'");
     ...
     * @return void
     */
    function ChangePassword(string $email,string $token,string $passwd):void {
        $arr=self::$db->Query("SELECT * FROM user WHERE email='$email' AND token='$token'");
        if (count($arr)==0) API_Controller::error_invalid_token();
        self::$db->Execute("UPDATE user SET password=");
    }
}
