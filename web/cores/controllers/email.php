...
class Email_Controller {
    ...
    static function SendEmail(array $email,string $title,string $content):void {
        ...
        try {
            ...
            $mail->Username=$config["email"]["email_name"];                // SMTP 用户名  即邮箱的用户名
            $mail->Password=$config["email"]["email_password"];             // SMTP 密码  部分邮箱是授权码(例如163邮箱)
            $mail->SMTPSecure=$config["email"]["email_protocol"];                    // 允许 TLS 或者ssl协议
            $mail->Port=$config["email"]["email_port"];                            // 服务器端口 25 或者465 具体要看邮箱服务器支持
            $mail->setFrom($config["email"]["email_name"],$config["email"]["email_from"]);
