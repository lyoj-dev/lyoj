...
function run(array $param,string &$html,string &$body):void {
    ...
    // $control_panel.=InsertTags("button",array("onclick"=>"location.href='".GetUrl("admin",array("page"=>"bash"))."'"),$lang["button"]["bash"]);
...
function user_list(array $param,string &$html,string &$body):void {
    ...
    $tmp.=InsertTags("div",array("style"=>InsertInlineCssStyle(array("width"=>"40%"))),
        InsertTags("a",array(
...
function contest_list_run(array $param,string &$html,string &$body):void {
    ...
    for ($i=0;$i<count($contest_list);$i++) {
        ...
        $tmp.=InsertTags("div",array("style"=>InsertInlineCssStyle(array("width"=>"37%"))),
        InsertTags("a",array(
...
function problem_list_run(array $param,string &$html,string &$body):void {
    ...
    for ($i=0;$i<count($problem_list);$i++) {
        ...
        InsertTags("a",array("href"=>GetUrl("problem",array("id"=>$problem_list[$i]["id"])),"class"=>"a"),"P".$problem_list[$i]["id"]));
        $tmp.=InsertTags("div",array("style"=>InsertInlineCssStyle(array("width"=>"35%"))),
            InsertTags("a",array(
    ...
    "style"=>InsertInlineCssStyle(array("margin-right"=>"10px"))),InsertTags("p",null,$lang["top"]));
    if ($page==1) $content.=InsertTags("div",array("class"=>"pages banned"),InsertTags("p",null,$lang["previous"]));
    else $content.=InsertTags("div",array("class"=>"pages","onclick"=>"location.href='".GetUrl("admin",array("page"=>"problem","num"=>$page-1,"key"=>$param["key"],"tag"=>$param["tag"],"diff"=>$param["diff"]))."'"),InsertTags("p",null,$lang["previous"]));
    if ($page==$pages_num) $content.=InsertTags("div",array("class"=>"pages banned"),InsertTags("p",null,$lang["next"]));
    else $content.=InsertTags("div",array("class"=>"pages","onclick"=>"location.href='".GetUrl("admin",array("page"=>"problem","num"=>$page+1,"key"=>$param["key"],"tag"=>$param["tag"],"diff"=>$param["diff"]))."'"),InsertTags("p",null,$lang["next"]));
