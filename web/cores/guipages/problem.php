...
function run(array $param,string &$html,string &$body):void {
    ...
    $body.=InsertTags("div",array("class"=>"default_main","style"=>InsertInlineCssStyle(array(
        ...
        $tmp.=InsertTags("div",array("style"=>InsertInlineCssStyle(array("width"=>"7%"))),InsertTags("a",array("class"=>"a","href"=>GetUrl("problem",array("id"=>$problem_list[$i]["id"]))),"P".$problem_list[$i]["id"]));
        $tmp.=InsertTags("div",array("style"=>InsertInlineCssStyle(array("width"=>"40%"))),InsertTags("a",array(
    ...
    else $content.=InsertTags("div",array("class"=>"pages","onclick"=>"location.href='".GetUrl("problem",array("page"=>$page-1,"key"=>$param["key"],"tag"=>$param["tag"],"diff"=>$param["diff"]))."'"),InsertTags("p",null,$lang["pages"]["previous"]));
    if ($page==$pages_num) $content.=InsertTags("div",array("class"=>"pages banned"),InsertTags("p",null,$lang["pages"]["next"]));
    else $content.=InsertTags("div",array("class"=>"pages","onclick"=>"location.href='".GetUrl("problem",array("page"=>$page+1,"key"=>$param["key"],"tag"=>$param["tag"],"diff"=>$param["diff"]))."'"),InsertTags("p",null,$lang["pages"]["next"]));
