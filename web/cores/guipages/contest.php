...
function info_run(array $param,string& $html,string& $body):void {
    ...
    ($info["starttime"]<=time()&&$contest_controller->JudgeSignup($param["id"]))))||$info["starttime"]+$info["duration"]<=time()) {
        ...
        $features.=InsertTags("button",array("onclick"=>"location.href='".GetAPIUrl("/contest/rank",array("id"=>$param["id"]))."'"),$lang["info"]["button"]["download-ranking"]);
    } $body.=InsertTags("div",array("class"=>"flex default_main","style"=>InsertInlineCssStyle(array(
    ...
    switch($param["page"]) {
        ...
        case "problem": {
            ...
            for ($i=0;$i<count($info["problem"]);$i++) {
                ...
                $tmp.=InsertTags("div",array("style"=>InsertInlineCssStyle(array("width"=>"40%","cursor"=>"pointer"))),
                    InsertTags("a",array(
        ...
        case "submission": {
            ...
            for ($i=0;$i<count($status);$i++) {
                ...
                $header.=InsertTags("div",array("style"=>InsertInlineCssStyle(array("width"=>"15%"))),
                InsertTags("a",array("href"=>GetUrl("user",array("id"=>$userinfo["id"])),"class"=>"ellipsis a"),$userinfo["name"]));
                $header.=InsertTags("div",array("style"=>InsertInlineCssStyle(array("width"=>"45%"))),
                InsertTags("a",array(
        ...
        case "ranking": {
            ...
            if ($contest_controller->JudgeSignup($param["id"])) {
                ...
                $tmp.=InsertTags("div",array("style"=>InsertInlineCssStyle(array("width"=>"20%"))),
                    InsertTags("a",array("href"=>GetUrl("user",array("id"=>$ranking[$id]["uid"])),"class"=>"ellipsis a"),$ranking[$id]["name"]));
                ...
                $tmp.=InsertTags("div",array("style"=>InsertInlineCssStyle(array("width"=>"20%"))),
                InsertTags("a",array("href"=>GetUrl("user",array("id"=>$ranking[$i]["uid"])),"class"=>"ellipsis a"),$ranking[$i]["name"]));
                $tmp.=InsertTags("div",array("class"=>"flex","style"=>InsertInlineCssStyle(array("width"=>"$percent%","justify-content"=>"center"))),
